...
EXT_GALLERY_JSON=''
if [ $USE_CUSTOM_EXTENSION_GALLERY -eq 1 ]; then
...
if grep -q "$CODE_SERVER_INSTALL_LOC/bin" "$JUPYTER_CONFIG_FILE"; then
...
# Define all required variables for the ec2-user environment
XDG_DATA_HOME="/home/ec2-user/SageMaker/.xdg/data"
XDG_CONFIG_HOME="/home/ec2-user/SageMaker/.xdg/config"
CODE_SERVER_INSTALL_LOC="/home/ec2-user/SageMaker/.cs"
LAB_3_EXTENSION_DOWNLOAD_URL="https://github.com/aws-samples/amazon-sagemaker-codeserver/releases/download/v0.2.0/sagemaker-jproxy-launcher-ext-0.2.0.tar.gz"
...
# Install uv and poetry (installed as ec2-user)
echo "Installing uv..."
curl -LsSf https://astral.sh/uv/install.sh | bash
echo "Installing poetry..."
curl -sSL https://install.python-poetry.org | python3 -

# Configure Code Server settings and keybindings
# Define settings file path.
CODE_SERVER_SETTINGS_FILE="\${XDG_DATA_HOME}/code-server/User/settings.json"
# Ensure the parent directory exists.
mkdir -p "\$(dirname "\$CODE_SERVER_SETTINGS_FILE")"
cat >"\$CODE_SERVER_SETTINGS_FILE" <<'EOFINNER'
{
...
# Create keybindings.json in the same directory
KEYBINDINGS_FILE="\${XDG_DATA_HOME}/code-server/User/keybindings.json"
cat >"\$KEYBINDINGS_FILE" <<'EOFINNER'
[
    {
        "key": "cmd+enter",
